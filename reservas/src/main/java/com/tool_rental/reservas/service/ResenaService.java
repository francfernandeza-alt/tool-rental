...
public class ResenaService {
    ...
    public List<ResenaDTO> obtenerTodos() {
        log.info("Obteniendo todas las rese̱as registradas");
        ...
        log.info("Total de rese̱as encontradas: {}", resenasDTO.size());
        return resenasDTO;
    }

    public ResenaDTO buscarPorId(Integer id) {
        log.info("Buscando rese̱a con ID {}", id);

        Resena resena = resenaRepository.findById(id).orElseThrow(() -> {
            log.error("No se encontro rese̱a con ID {}", id);
            return new RuntimeException("No se encontro la rese̱a con el ID ingresado.");
        });
        return convertirADTO(resena);
    }

    public List<ResenaDTO> buscarPorRutUsuario(String rutUsuario) {
        log.info("Buscando rese̱as asociadas al usuario {}", rutUsuario);
        ...
        log.info("Se encontraron {} rese̱as para el usuario {}", resenasDTO.size(), rutUsuario);
        return resenasDTO;
    }

    public List<ResenaDTO> buscarPorHerramientaId(Integer herramientaId) {
        log.info("Buscando rese̱as asociadas a la herramienta {}", herramientaId);
        ...
        log.info("Se encontraron {} rese̱as para la herramienta {}", resenasDTO.size(), herramientaId);
        return resenasDTO;
    }

    public List<ResenaDTO> buscarPorReservaId(Integer reservaId) {
        log.info("Buscando rese̱as asociadas a la reserva {}", reservaId);
        ...
        log.info("Se encontraron {} rese̱as para la reserva {}", resenasDTO.size(), reservaId);
        return resenasDTO;
    }

    public ResenaDTO guardar(Resena resena) {
        log.info("Registrando nueva rese̱a para reserva {} y herramienta {}",
                resena.getReservaId(),
        ...
        if (resena.getFechaResena() == null) {
            ...
            log.info("Fecha de rese̱a asignada automaticamente: {}", resena.getFechaResena());
        }
        Resena resenaGuardada = resenaRepository.save(resena);

        log.info("Rese̱a registrada correctamente con ID {}", resenaGuardada.getIdResena());
        return convertirADTO(resenaGuardada);
    }

    public ResenaDTO actualizar(Integer id, Resena resena) {
        log.info("Actualizando rese̱a con ID {}", id);
        ...
        log.info("Rese̱a con ID {} actualizada correctamente", resenaActualizada.getIdResena());
        return convertirADTO(resenaActualizada);
    }

    public String eliminar(Integer id) {
        log.info("Intentando eliminar rese̱a con ID {}", id);
        ...
        return "La rese̱a con ID " + id + " fue eliminada correctamente.";
    }
